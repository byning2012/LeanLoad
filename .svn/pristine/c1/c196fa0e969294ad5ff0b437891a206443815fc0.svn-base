...
//    ImageView mErrorIv;
    ...
    public void repayMent() {
        ...
        } else if ("3".equals(mHomeModel.getData().getStatus())) {
            showToast("审核未通过，请修改", Toast.LENGTH_SHORT);
            intent.setClass(getActivity(), BaseCertificationActivity.class);
            startActivity(intent);
        } else if ("2".equals(mHomeModel.getData().getStatus())) {
            if (Double.parseDouble(mHomeModel.getData().getStill_money()) < 100) {
                showToast("额度不足,请提升额度", Toast.LENGTH_SHORT);
        ...
        } else if ("4".equals(mHomeModel.getData().getStatus())) {
