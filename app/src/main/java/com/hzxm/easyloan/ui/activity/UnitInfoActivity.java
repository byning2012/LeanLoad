...
import com.hzxm.easyloan.presenter.implPresenter.UnitInfoPresenter;
 ...
 * 单位信息
 */
public class UnitInfoActivity extends BaseActivity implements
        IPermissionResultListener, GalleryFinal.OnHanlderResultCallback, IUnitInfoView {
    ...
    private AmountCreateModel.DataEntity.InfoEntity.UnitEntity mInfoEntity;
    List<AmountCreateModel.DataEntity.InfoEntity.UnitEntity.UnitImgEntity> mImages;
    ...
    protected void initData() {
        try {
            ...
            etName.setText(mInfoEntity.getUnit_name());
            etName.setSelection(mInfoEntity.getUnit_name().length());
            etPlane.setText(mInfoEntity.getUnit_tel());
            etWorkPosition.setText(mInfoEntity.getUnit_work());
            etWorkAge.setText(mInfoEntity.getUnit_work_year());
            etEntryTime.setText(mInfoEntity.getUint_work_time());
            etSalary.setText(mInfoEntity.getUnit_money());
            ...
            if (mImages.get(2) != null && !"".equals(mImages.get(2).getImage())) {
        ...
        mUnitInfoPresenter = new UnitInfoPresenter(this);
        ...
        switch (rl.getId()) {
            case R.id.rl_updata1:
                ...
                permissionRequest("是否允许凡易贷打开相机",
                        ...
                        101, UnitInfoActivity.this);
                break;
            case R.id.rl_updata2:
                type = 2;
                permissionRequest("是否允许凡易贷打开相机",
                        new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.CAMERA},
                        102, UnitInfoActivity.this);
                break;
            case R.id.rl_updata3:
                type = 3;
                permissionRequest("是否允许凡易贷打开相机",
                        new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.CAMERA},
                        103, UnitInfoActivity.this);
                break;
        }
    }
    ...
    public void onPermissionSuccess() {
        popupWindow = new DocumentPopupWindow(UnitInfoActivity.this, rlUpdata1);
    ...
    public void onHanlderSuccess(int reqeustCode, List<PhotoInfo> resultList) {
         ...
         * 压缩图片
         */
        Luban.get(UnitInfoActivity.this)
                .load(file)
                ...
                .subscribe(new Action1<File>() {
                    ...
                    public void call(File file) {
                        ...
                        mUnitInfoPresenter.uploadFace(file, uid + "", hashid, finalType);
                    }
                });
    ...
    public void submitInfo() {
        ...
        if ("".equals(companyName)) {
            showToast("请输入单位名称", Toast.LENGTH_SHORT);
            return;
        }
        String planeTel = etPlane.getText().toString().trim();
        if ("".equals(planeTel)) {
            showToast("请输入单位座机", Toast.LENGTH_SHORT);
            return;
        }
        ...
        if ("".equals(path2)) {
            showToast("请上传单位照片", Toast.LENGTH_SHORT);
            return;
        }
        ...
        mUnitInfoPresenter.unitInfo(uid + "", companyName, planeTel, mPositionInfo, mWorkAge, mWorkTime, mSalary, mPics.toString());
    }
    ...
    public class DocumentPopupWindow extends PopupWindow {
        public DocumentPopupWindow(Context mContext, View parent) {
            View view = View.inflate(UnitInfoActivity.this, R.layout.pop_camera_select, null);
            view.findViewById(R.id.tv_photo).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    GalleryFinal.openGallerySingle(Constant.REQUEST_CODE_GALLERY, UnitInfoActivity.this);
                    popupWindow.dismiss();
                }
            });
            view.findViewById(R.id.tv_camera).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    GalleryFinal.openCamera(Constant.REQUEST_CODE_CAMERA, UnitInfoActivity.this);
                    popupWindow.dismiss();
                }
            });
    ...
    protected void onDestroy() {
        super.onDestroy();
        mUnitInfoPresenter.unsubcrible();
    }
}
